...
const FriendsPage = ({ socket, setHasNewFriendRequest, themeMode = 'light' }) => {
  ...
  if (isLoading) return <p className={`text-center mt-8 ${isDark ? 'text-gray-300' : ''}`}>Loading...</p>;


  return (
    <div className="max-w-3xl mx-auto p-4 sm:p-6 font-sans h-full flex flex-col">
      <h1 className={`text-3xl font-bold mb-6 ${isDark ? 'text-white' : 'text-gray-800'}`}>Friends</h1>
      
      {}
      <div className={`flex items-center gap-2 border-b mb-6 ${isDark ? 'border-zinc-700' : ''}`}>
        ...
        {activeTab === 'friends' && (
          <div className="space-y-2">
            {friends.length > 0 ? friends.map(user => (
              <div key={user.id} className={`flex justify-between items-center p-3 rounded-lg transition-colors ${isDark ? 'hover:bg-zinc-900' : 'hover:bg-gray-100'}`}>
                <Link to={`/profile/${user.username}`} className="flex items-center gap-4 hover:underline">
                  <Avatar user={user} size="w-12 h-12" />
                  <span className={`font-semibold ${isDark ? 'text-gray-100' : 'text-gray-700'}`}>{user.username}</span>
                </Link>
        ...
        {activeTab === 'invites' && (
          <div className="space-y-3">
            {groupInvites.length > 0 ? groupInvites.map(invite => (
              <div key={invite.id} className={`p-4 rounded-lg border ${isDark ? 'bg-zinc-900 border-zinc-700 text-gray-100' : 'bg-gray-50 border-gray-200'}`}>
