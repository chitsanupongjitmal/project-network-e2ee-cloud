...
const PrivateChatPage = ({ socket, currentUser, keyPair, peerKeyVersions, onlineFriends }) => {
    ...
    const {
        ...
        isUploading
    } = usePrivateChat(peerUsername, socket, currentUser, keyPair, peerKeyVersions, onlineFriends);

    const { 
        call, callAccepted, callDuration, remoteStream,
        isMuted, toggleMute, callUser, answerCall, endCall
    ...
    const startAudioCall = useCallback(() => {
        ...
        callUser(peerUser.id, 'audio', {
            ...
            displayName: peerUser.nickname || peerUser.username,
            avatarUrl: peerUser.avatar_url || null,
        });
    }, [peerUser, callUser]);

    const acceptCallWrapper = useCallback(() => answerCall(), [answerCall]);
    const endCallWrapper = useCallback(() => endCall(), [endCall]);
    ...
    return (
        <div className="flex flex-col flex-1 h-full max-h-screen overflow-y-hidden bg-white">
            ...
            <div className="w-full bg-white p-2 border-t">
                 {(isBlockedByMe || hasBlockedMe) ? (
                    <div className="p-4 text-center text-gray-500">
                        {isBlockedByMe ? `You have blocked ${peerUsername}.` : `${peerUsername} has blocked you.`}
                    </div>
                ) : (
                    <MessageInput 
                        onSend={handleSend}
            ...
            <CallModal
                ...
                answerCall={acceptCallWrapper}
                endCall={endCallWrapper}
